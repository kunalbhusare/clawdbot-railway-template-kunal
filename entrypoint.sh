...
if [ -d /wrapper/clawlifeos/skills ]; then
  mkdir -p /root/.openclaw/skills
  cp -r /wrapper/clawlifeos/skills/* /root/.openclaw/skills/
  echo "[entrypoint] Skills installed to /root/.openclaw/skills/"
fi

# 2. Start Linear webhook receiver via pm2
#    Uses baked-in ecosystem config (from /wrapper/clawlifeos/) or persistent one (from /data/clawlifeos/)
if [ -f /wrapper/clawlifeos/ecosystem.config.js ]; then
  pm2 start /wrapper/clawlifeos/ecosystem.config.js --silent || echo "[entrypoint] WARN: pm2 start failed (baked-in config)"
